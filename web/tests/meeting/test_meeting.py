...
def test_delete_old_meetings(
    ...
    mocker.patch("b3desk.models.bbb.BBB.delete_all_recordings", return_value={})
...
def test_delete_old_meetings_keeps_meeting_when_bbb_fails(
    mocker, client_app, time_machine, meeting, user
):
    """An expired meeting whose recordings BBB refuses to delete is kept."""
    mocker.patch(
        "b3desk.models.bbb.BBB.delete_all_recordings",
        return_value={"returncode": "FAILED", "message": "some error"},
    )
    ...
    assert meeting in user.meetings
